from ProvisioningManager.ODL2_plugin.odl_objects.action import Action
...
from ProvisioningManager.ODL2_plugin.odl2_api import ODL2_api
from ProvisioningManager.ODL2_plugin.flow_requester import FlowRequester
...
class ODL2_plugin:
    ...
    def __init__(self, **kwargs):
        for key in (
        'ctl_addr', 'ctl_port', 'ctl_user', 'ctl_password', 'ctl_nthreads', 'ctl_operativemode', 'ctl_flowmeters'):
            if key in kwargs:
                setattr(self, key.split('_')[1], kwargs[key])

        # API Initialization

        self.api = ODL2_api(self.user, self.password, self.addr,
                            self.port)
        self.name = 'ODL2'
    ...
    def create_connection(self, connection):
        try:
            ...
            logger.info("Sending new connection to ODL: " + str(connection))
    ...
    def remove_connection(self, connection, id=None):
        try:
            logger.info("Sending new connection to remove in ODL: " + str(connection))
